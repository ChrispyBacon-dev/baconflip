...
class FinanceCog(commands.Cog):
    ...
    async def stock(self, ctx: commands.Context, symbol: str):
        ...
        if not symbol:
            await ctx.send(embed=create_error_embed("Missing Argument", "Please provide a stock symbol (e.g., AAPL)."))
            return

        symbol = symbol.upper()
        await ctx.send(f"⏳ Fetching stock data for **{symbol}**...")
        ...
        try:
            ...
            if price is None:
                 await ctx.message.edit(content=None, embed=create_error_embed(
                    "Data Error",
                    f"Received incomplete data for **{symbol}**. Price information is missing."
                ))
                 return
            ...
            embed = nextcord.Embed(
                ...
                color=nextcord.Color.green() if float(change or 0) >= 0 else nextcord.Color.red()
            )
            embed.add_field(name="Price", value=f"${float(price):.2f}", inline=True)
            embed.add_field(name="Change", value=f"{float(change):.2f} ({change_percent})", inline=True)
    ...
    async def crypto(self, ctx: commands.Context, crypto_symbol: str, fiat_symbol: str = 'USD'):
        ...
        if not crypto_symbol:
            await ctx.send(embed=create_error_embed("Missing Argument", "Please provide a crypto symbol (e.g., BTC)."))
            return
        ...
        await ctx.send(f"⏳ Fetching exchange rate for **{pair}**...")
